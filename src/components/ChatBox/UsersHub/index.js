...
import { CHAT_BOX_MY_CHATS_CLICK, CHAT_BOX_USERS_HUB_CLICK, CHAT_BOX_USER_ROW_SELECT } from '../../../redux/actionTypes';
...
const UsersHub = () => {
  ...
  const { usersList, loggedInUserDetails: {id} } = chatBox;
  ...
  return (
    <div className="users-hub-container">
      {usersList.map((user) => {
        ...
        return (
          <div className="user-container" key={user.id} onClick={() => onUserRowClick(user)}>
            ...
            <div>
              {!isLoggedInUser && <ChevronRightOutlinedIcon />}
            </div>
          </div>
        );
      })}
